...
public class DeviceDiscoveryService {
    ...
    private static final BroadlinkLogger log = BroadlinkLogger.getLogger("BroadlinkDevice");
    ...
    public CompletableFuture<List<BroadlinkDevice>> discover(int port, long timeout) {
        ...
        CompletableFuture.delayedExecutor(timeout, TimeUnit.MILLISECONDS).execute(() -> {
            ...
            log.debug("Discovery complete: " + devices.size() + " devices found");
        ...
        try {
            ...
            while (!socket.isClosed()) {
                try {
                    ...
                    devices.add(device);
                    log.debug("Discovered: " + name + " (0x" + Integer.toHexString(deviceType) + ")");
